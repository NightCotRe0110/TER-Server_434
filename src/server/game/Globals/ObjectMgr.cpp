...
void ObjectMgr::LoadGossipMenuItemsLocales()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %lu ïåðåâîäîâ äëÿ ìåíþ ïåðñîíàæåé çà %u ìñ", (unsigned long)_gossipMenuItemsLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPointOfInterestLocales()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %lu ïåðåâîäîâ òî÷êè òåêñòà ìîíñòðîâ çà %u ìñ", (unsigned long)_pointOfInterestLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ 0 õàðàêòåðèñòèê ñóùåñòâ. òàáëèöà `creature_template` ïóñòà.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ñóùåñòâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureTemplateAddons()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u îðóæèÿ è ïðåäìåòîâ äëÿ ñóùåñòâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureAddons()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u áðîíÿ è îðóæèå íà ñóùåñòâàõ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadEquipmentTemplates()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u õàðàêòåðèñòèê äëÿ ñóùåñòâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureModelInfo()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ìîäåëåé äëÿ ñóùåñòâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadLinkedRespawn()
{
   ...
   //     sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 linked respawns. DB table `linked_respawn` is empty.");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍ(î) " UI64FMTD " âðåìÿ âîñòàíîâëåíèÿ çà  %u ìñ", uint64(_linkedRespawnStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatures()
{
  ...
  //      sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 creatures. DB table `creature` is empty.");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ìîíñòðîâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
uint32 ObjectMgr::AddGOData(uint32 entry, uint32 mapId, float x, float y, float z, float o, uint32 spawntimedelay, float rotation0, float rotation1, float rotation2, float rotation3)
{
    ...
    sLog->outDebug(LOG_FILTER_MAPS, "ÇÀÌÅ×ÅÍÎ: ïåðñîíàæ %u íîìåðîì %u íà êàðòå %u x %f y %f z %f o %f", guid, entry, mapId, x, y, z, o);
...
void ObjectMgr::LoadGameobjects()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %lu îáúåêòîâ çà %u ìñ", (unsigned long)_gameObjectDataStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemLocales()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %lu íàçâàíèé ïðåäìåòîâ çà %u ìñ", (unsigned long)_itemLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ïðåäìåòîâ èç Item-sparse.db2 è %u èç ÁÄ çà %u ìñ", sparseCount, dbCount, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemTemplateAddon()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ôóíêöèé ïðåäìåòîâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemScriptNames()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ñêðèïòîâ äëÿ ïðåäìåòîâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadVehicleTemplateAccessories()
{
  ...
  //          sLog->outError(LOG_FILTER_SQL, "Table `vehicle_template_accessory`: Accessory %u does not exist.", uiAccessory);
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ìàøèí çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadVehicleAccessories()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u õàðàêòåðèñòèêè òåõíèêè çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPetLevelInfo()
{
  ...
  //      sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 level pet stats definitions. DB table `pet_levelstats` is empty.");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ñâåäåíèÿ äëÿ óðîâíåé ïåòîìöåâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPlayerInfo()
{
    // Load playercreate
    {
        ...
        else
        {
            ...
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ïåðñîíàæåé çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Load playercreate items
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ÇÀÃÐÓÇÊÀ ôàéëîâ...");
    {
        uint32 oldMSTime = getMSTime();
        //                                                0     1      2       3
        ...
        else
        {
            ...
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ïðåäìåòîâ ó ïåðñîíàæåé çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Load playercreate spells
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ÇÀÃÐÓÇÊÀ ÇÀÊËÈÍÀÍÈÉ...");
    {
        uint32 oldMSTime = getMSTime();
      ...
      //      sLog->outError(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 player create spells. DB table `%s` is empty.", sWorld->getBoolConfig(CONFIG_START_ALL_SPELLS) ? "playercreateinfo_spell_custom" : "playercreateinfo_spell");
        ...
        else
        {
            ...
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u çàêëèíàíèé ó ïåðñîíàæåé çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Load playercreate actions
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ÇÀÃÐÓÇÊÀ ÀÊÒÈÎÍÎÂ...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        else
        {
           ...
           //         sLog->outError(LOG_FILTER_SQL, "Wrong race %u in `playercreateinfo_action` table, ignoring.", current_race);
            ...
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u àêòèîíû ïåðñîíàæåé çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Loading levels data (class/race dependent)
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ÇÀÃÐÓÇÊÀ ÓÐÎÂÍÅÉ ÏÅÐÑÎÍÀÆÅÉ...");
    {
        uint32 oldMSTime = getMSTime();
...
//                    qinfo->GetQuestId(), qinfo->ZoneOrSort);
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %lu ïðîâåðêà çàäàíèé çà %u ìñ", (unsigned long)_questTemplates.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadQuestLocales()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %lu Ïåðåâîäîâ çàäàíèé çà %u ìñ", (unsigned long)_questLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadScripts(ScriptsType type)
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "ÇÀÃÐÓÆÅÍÎ %s...", tableName.c_str());
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ñêðèïòîâ îáðàáîòîê çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadSpellScriptNames()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ñêðèïòîâ íà çàêëèíàíèÿ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::ValidateSpellScripts()
{
    ...
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÏÐÎÂÅÐÅÍÎ 0 ñêðèïòîâ.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÏÐÎÂÅÐÅÍÎ %u ñêðèïòîâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPageTexts()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u êíèã çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadInstanceTemplate()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ïîäçåìåëèé çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGossipText()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ñëîâ äëÿ ñóùåñòâ", count);
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u òåêñòîâ ñóùåñòâ òîðãîâëè çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadNpcTextLocales()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %lu ïåðåâîä òåñòà ñóùåñòâ çà %u ìñ", (unsigned long)_npcTextLocaleStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::ReturnOrDeleteOldMails(bool serverUp)
{
    ...
    sLog->outInfo(LOG_FILTER_GENERAL, "ÏÅÐÅÇÀÏÓÑÊ ñðîêà õðàíåíèÿ ïèñåì: ÷àñû: %d, ìèíóòû: %d, ñåêóíäû: %d ", lt->tm_hour, lt->tm_min, lt->tm_sec);
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Íå íàéäåíî èñòåêøèõ ñêðîê ïèñåì.");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÎÁÐÀÁÎÒÀÍÎ %u çàïðîñîâ ïî÷òû: %u óäàëåíî %u âîñòàíîâëåíî çà %u ìñ", deletedCount + returnedCount, deletedCount, returnedCount, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadQuestAreaTriggers()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ïóòåé çàäàíèé çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadTavernAreaTriggers()
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u òàâåðí çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadAreaTriggerScripts()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ 0 àðåà-òðèãåðíûõ ñêðèïòîâ. Òàáëèöà `areatrigger_scripts` ïóñòà.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u àðåà-òðèãåðíûõ ñêðèïòîâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGraveyardZones()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ 0 çîí êëàäáèù. òàáëèöà `game_graveyard_zone` ïóñòà.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u çîí êëàäáèù çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadAreaTriggerTeleports()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ 0 ïëîùàäîê òåëåïîðòîâ. òàáëèöà  `areatrigger_teleport` ïóñòà.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u ïëîùàäêè òåëåïîðòîâ çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadAccessRequirements()
{
    ...
    if (!result)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ 0 òðåáîâàíèé äîñòóïà îïðåäåëåíèé. Òàáëëèöû `access_requirement` ïóñòà.");
        return;
    }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> ÇÀÃÐÓÆÅÍÎ %u òðåáîâàíèÿ äîñòóïà îïðåäåëåíèé çà %u ìñ", count, GetMSTimeDiffToNow(oldMSTime));
}
...
const char *ObjectMgr::GetTrinityString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        ...
        sLog->outError(LOG_FILTER_SQL, "Entry %i not found in `ter_string` table.", entry);
    else
        sLog->outError(LOG_FILTER_SQL, "TER-Core string entry %i not found in DB.", entry);
