...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
	{
		...
		{
			player->ADD_GOSSIP_ITEM(10974, 0, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
		}
			...
			if (spell->Id == SPELL_RAPTOR_ROPE)
				if (Player* player = caster->ToPlayer())
					if (player->GetQuestStatus(QUEST_YOUNG_AND_VICIOUS) == QUEST_STATUS_INCOMPLETE)
						...
						{
							player->KilledMonsterCredit(NPC_SWIFTCLAW);
							if (Creature* npc = player->SummonCreature(NPC_SWIFTCLAW_SUMMON, me->GetPositionX() + 1.0f, me->GetPositionZ(), 0.0f))
