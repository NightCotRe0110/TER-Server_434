...
enum MageSpells
{
	...
	SPELL_MAGE_MOLTEN_FURY_RANK1                 = 31679,
	SPELL_MAGE_MOLTEN_FURY_RANK2                 = 31680,
	SPELL_MAGE_MOLTEN_FURY_RANK3                 = 86880,
	SPELL_MAGE_FIRE_POWER_RANK1                  = 18459,
	SPELL_MAGE_FIRE_POWER_RANK2                  = 18460,
	SPELL_MAGE_FIRE_POWER_RANK3                  = 54734,
	SPELL_MAGE_CRITICAL_MASS_RANK1               = 11095,
	SPELL_MAGE_CRITICAL_MASS_RANK2               = 12872,
	SPELL_MAGE_CRITICAL_MASS_RANK3               = 12873,
	...
	class spell_mage_ice_lance_SpellScript : public SpellScript
	{
		...
		void HandleOnHit()
		{
			if (Player* caster = GetCaster()->ToPlayer())
			{
				...
				{
					//Forst Mastery Bonus.
					MasteryRating = caster->GetMasteryPoints();
					if (caster->GetPrimaryTalentTree(caster->GetActiveSpec()) == TALENT_TREE_MAGE_FROST)
						mb = (1.0f + MasteryRating * 0.025f);
					else
						mb = 1.0f;
					//Spell Power.
					int32 sp = caster->SpellBaseDamageBonusDone(GetSpellInfo()->GetSchoolMask());
					if (unitTarget->HasAuraState(AURA_STATE_FROZEN, GetSpellInfo(), GetCaster()))
					{
						SetHitDamage((irand(351, 447) + int32(0.378f * sp)) * 2.0f * mb);
					}
					else
					{
						SetHitDamage(irand(351, 447) + int32(0.378f * sp));
					}
				}
			}
		}
...
/*######
## spell_mage_living_bomb_tic  - 44457
######*/

class spell_mage_living_bomb_tic : public SpellScriptLoader
{
public:
	spell_mage_living_bomb_tic() : SpellScriptLoader("spell_mage_living_bomb_tic") { }

	class spell_mage_living_bomb_tic_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_mage_living_bomb_tic_AuraScript);

		int32 damage;
		uint32 SpellBonusDamage;
		float FirePowerTalentRank;
		float CriticalMassTalent;
		float MasteryRating;
		float moltenfury;
		float haste;
		...
		void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
		{
			...
			if (caster && target)
			{
				...
				{
					if (caster->HasTalent(SPELL_MAGE_MOLTEN_FURY_RANK1, caster->GetActiveSpec()))
						moltenfury = 1.04f;
					else if (caster->HasTalent(SPELL_MAGE_MOLTEN_FURY_RANK2, caster->GetActiveSpec()))
						moltenfury = 1.08f;
					else if (caster->HasTalent(SPELL_MAGE_MOLTEN_FURY_RANK3, caster->GetActiveSpec()))
						moltenfury = 1.12f;
					else
						moltenfury = 1.0f;
				}
		...
		void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool& canBeRecalculated)
		{
			...
			{
				//Mastery
				MasteryRating = caster->ToPlayer()->GetMasteryPoints();
				//SpellBonusDamage
				SpellBonusDamage = caster->SpellBaseDamageBonusDone(GetSpellInfo()->GetSchoolMask());
				//Haste
				haste = caster->GetRatingBonusValue(CR_HASTE_SPELL);
				//FirePowerTalentRank scaling
				if (caster->HasTalent(SPELL_MAGE_FIRE_POWER_RANK1, caster->GetActiveSpec()))
					FirePowerTalentRank = 1.01f;
				else if (caster->HasTalent(SPELL_MAGE_FIRE_POWER_RANK2, caster->GetActiveSpec()))
					FirePowerTalentRank = 1.02f;
				else if (caster->HasTalent(SPELL_MAGE_FIRE_POWER_RANK3, caster->GetActiveSpec()))
					FirePowerTalentRank = 1.03f;
				else
					FirePowerTalentRank = 1.0;
				//CriticalMassTalent scalig
				if (caster->HasTalent(SPELL_MAGE_CRITICAL_MASS_RANK1, caster->GetActiveSpec()))
					CriticalMassTalent = 1.05f;
				else if (caster->HasTalent(SPELL_MAGE_CRITICAL_MASS_RANK2, caster->GetActiveSpec()))
					CriticalMassTalent = 1.10f;
				else if (caster->HasTalent(SPELL_MAGE_CRITICAL_MASS_RANK3, caster->GetActiveSpec()))
					CriticalMassTalent = 1.15f;
				else
					CriticalMassTalent = 1.0f;

				damage = (232 + (SpellBonusDamage * 0.258f)) * (1.25f) * (0.028f * MasteryRating + 1.0f) * (FirePowerTalentRank)* (CriticalMassTalent);
			}
		}
		...
		{
			//Set Damage
			aurEff->SetAmount(damage * moltenfury * (1 / (1.0f - haste * 0.01f)));
		}

		void Register()
		{
			OnEffectApply += AuraEffectApplyFn(spell_mage_living_bomb_tic_AuraScript::OnApply, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL);
			DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_mage_living_bomb_tic_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
			OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_mage_living_bomb_tic_AuraScript::HandleEffectPeriodicUpdate, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
		}
	};

	AuraScript* GetAuraScript() const
	{
		return new spell_mage_living_bomb_tic_AuraScript();
	}
};

/*######
## spell_mage_combustion_tic  - 83853
######*/

class spell_mage_combustion_tic : public SpellScriptLoader
{
public:
	spell_mage_combustion_tic() : SpellScriptLoader("spell_mage_combustion_tic") { }

	class spell_mage_combustion_tic_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_mage_combustion_tic_AuraScript);
		...
		void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
		{
			...
			//Add Ignite Damage
			if (AuraEffect const* aurEff = target->GetAuraEffect(SPELL_MAGE_IGNITE, EFFECT_0, caster->GetGUID()))
				igdamage = aurEff->GetAmount();
			else
				igdamage = 0;
			//Add Living Bomb Damage
			if (target->HasAura(SPELL_MAGE_LIVING_BOMB))
				lb = true;
			else
				lb = false;
			//Add PyroBlast Damage
			if (target->HasAura(SPELL_MAGE_PYROBLAST))
				pb = true;
			else
				pb = false;
		}
		...
		void HandleEffectPeriodicUpdate(AuraEffect* aurEff)
		{
			//Living Bomb Damage
			if (lb)
				lbdamage = (232 + (SpellBonusDamage * 0.258f)) * (1.25f) * (0.028f * MasteryRating + 1.0f) * (FirePowerTalentRank)* (CriticalMassTalent);
			else
				lbdamage = 0;
			//PyroBlast Damage
			if (pb)
				pbdamage = (162 + (SpellBonusDamage * 0.180f)) * (1.25f) * (0.028f * MasteryRating + 1.0f) * (FirePowerTalentRank);
			else
				pbdamage = 0;
			//Set Damage
			aurEff->SetAmount(((pbdamage / (3 * (1.0f - haste * 0.01f))) + (lbdamage / (3 * (1.0f - haste * 0.01f))) + (igdamage / 2)) * moltenfury);
		}

		void Register()
		{
			OnEffectApply += AuraEffectApplyFn(spell_mage_combustion_tic_AuraScript::OnApply, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL);
			DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_mage_combustion_tic_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
			OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_mage_combustion_tic_AuraScript::HandleEffectPeriodicUpdate, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
		}
	};

	AuraScript* GetAuraScript() const
	{
		return new spell_mage_combustion_tic_AuraScript();
	}
};
...
void AddSC_mage_spell_scripts()
{
	...
	new spell_mage_living_bomb_tic();
	new spell_mage_combustion_tic();
